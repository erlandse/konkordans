...
function loadIndexes() {
    for (var temp = 0; temp < indexList.length; temp++) {
        ToolsCorpus.addOption(document.getElementById("idIndexSelect"), indexList[temp].displayName, indexList[temp].index);
    }
...
function changeClick(id) {
    ...
    if (el.options[0].selected == true) {
        for (var temp = 1; temp < el.options.length; temp++)
...
function addMetadataToQuery(query) {
    ...
    for (var temp = 0; temp < curIndex.aggregations.length; temp++) {
        ...
        if (curIndex.aggregations[temp].multiValued == false) {
            ...
            if (arr.length == 0)
                continue;
            var termOr = JSON.parse("{\"bool\":{\"must\":[],\"should\":[],\"must_not\":[]}}");
            for (var i = 0; i < arr.length; i++) {
                var t = new Object();
                t.term = new Object();
