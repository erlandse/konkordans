...
function startUp() {
    ...
    formData.resturl = ToolsCorpus.gup("resturl");
    document.getElementById("spanSelect").value = "5";
    postPhpContext(formData, loadContextInContext);
}
...
function loadContextInContext(data) {
    ...
    formData.elasticdata = JSON.stringify(ob, null, 2);
    formData.resturl = ToolsCorpus.gup("resturl");
    postPhpContext(formData, writeOut);
}
...
function postPhpContext(formData, callBack) {
