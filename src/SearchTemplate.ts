class SearchTemplate {
    ...
    qRawText = true;
    jumpWordEnabled:boolean=false;
    debugger:boolean = false;
    constructor(searchString: string, p:any) {
       this.posArray = p; 
...
qMatch(wl,pos,posList,lemmaList){
  let temp:number;
  for(temp =0; temp < this.candidateList.length;temp++){
    ...
    if(this.candidateList[temp].type == this.WILDCARD){
       if(this.candidateList[temp].removable == true && this.candidateList[temp].isRemoved == true) {//the position is not moved
         continue;
       }  
